...
define([ 'jquery', 'area' ], function($, Area) {
  var Map = Class.extend({
    ...
    _loadMap : function(useWorker) {
      var self = this,
          filepath =
              "http://cdn.mozilla.net/browserquest/maps/world_client.json";
    ...
    _initTilesets : function() {
      var tileset1, tileset2, tileset3,
          CDN = "http://cdn.mozilla.net/browserquest/";
    ...
    GridPositionToTileIndex : function(x,
                                       y) { return (y * this.width) + x + 1; },
    ...
    /**
     * Returns true if the given position is located within the dimensions of
     * the map.
     *
     ...
     */
    isHighTile : function(id) { return _.indexOf(this.high, id + 1) >= 0; },
    ...
    getDoorDestination : function(
        x, y) { return this.doors[this.GridPositionToTileIndex(x, y)]; },
    ...
    getCurrentCheckpoint : function(entity) {
      return _.detect(
          this.checkpoints,
          function(checkpoint) { return checkpoint.contains(entity); });
    }
  });
