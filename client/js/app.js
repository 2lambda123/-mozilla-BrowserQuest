...
define([ 'jquery', 'storage' ], function($, Storage) {
  var App = Class.extend({
    init : function() {
      ...
      this.watchNameInputInterval =
          setInterval(this.toggleButton.bind(this), 100);
    ...
    center : function() { window.scrollTo(0, 1); },
    ...
    start : function(username) {
      ...
      if (username && !this.game.started) {
        var optionsSet = false, config = this.config;
    ...
    setMouseCoordinates : function(event) {
      var gamePos = $('#container').offset(),
          ...
          height = this.game.renderer.getHeight(), mouse = this.game.mouse;
    ...
    blinkHealthBar : function() {
      ...
      setTimeout(function() { $hitpoints.removeClass('white'); }, 500)
    },
    ...
    resetPage : function() {
      var self = this, $achievements = $('#achievements');

      if ($achievements.hasClass('active')) {
        $achievements.bind(TRANSITIONEND, function() {
          $achievements.removeClass('page' + self.currentPage)
              .addClass('page1');
          self.currentPage = 1;
    ...
    initEquipmentIcons : function() {
      ...
      var getIconPath =
              function(spriteName) {
        return 'http://cdn.mozilla.net/browserquest/img/' + scale + '/item-' +
               spriteName + '.png';
      },
          weapon = this.game.player.getWeaponName(),
          armor = this.game.player.getSpriteName(),
          weaponPath = getIconPath(weapon), armorPath = getIconPath(armor);
    ...
    showAchievementNotification : function(id, name) {
      var $notif = $('#achievement-notification'), $name = $notif.find('.name'),
      ...
      if (this.game.storage.getAchievementCount() === 1) {
        this.blinkInterval =
            setInterval(function() { $button.toggleClass('blink'); }, 500);
      }
      setTimeout(function() {
    ...
    displayUnlockedAchievement : function(id) {
      ...
      if (achievement && achievement.hidden) {
        this.setAchievementData($achievement, achievement.name,
                                achievement.desc);
      }
    ...
    initAchievementList : function(achievements) {
      var self = this, $lists = $('#lists'), $page = $('#page-tmpl'),
          $achievement = $('#achievement-tmpl'), page = 0, count = 0, $p = null;
    ...
    initUnlockedAchievements : function(ids) {
      ...
      _.each(ids, function(id) { self.displayUnlockedAchievement(id); });
    ...
    togglePopulationInfo :
        function() { $('#population').toggleClass('visible'); },

    openPopup : function(type, url) {
      var h = $(window).height(), w = $(window).width(), popupHeight,
          popupWidth, top, left;
    ...
    animateParchment : function(origin, destination) {
      var self = this, $parchment = $('#parchment'), duration = 1;
      ...
      if (this.isMobile) {
        ...
        if (this.isParchmentReady) {
          ...
          setTimeout(
              function() { self.isParchmentReady = !self.isParchmentReady; },
              duration * 1000);
        }
      }
    },
    ...
    showMessage : function(message) {
      ...
      this.messageTimer =
          setTimeout(function() { $wrapper.addClass('top'); }, 5000);
    },

    resetMessageTimer : function() { clearTimeout(this.messageTimer); },
