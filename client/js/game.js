define([
  ...
  var Game = Class.extend({
    ...
    connect: function (started_callback) {
      ...
      if (config) {
        this.client.connect(config.dispatcher); // false if the client connects directly to
        // a game server
        connecting = true;
      }
      //>>excludeEnd("prodHost");
      ...
      this.client.onWelcome(function (id, name, x, y, hp) {
        ...
        self.client.onSpawnCharacter(
          function (entity, x, y, orientation, targetId) {
                ...
                if (entity.id !== self.playerId) {
                  ...
                  if (entity instanceof Character) {
                      ...
                      if (!entity.isDying) {
                        ...
                        if (entity instanceof Player) {
                          ...
                          if (self.map.isDoor(gridX, gridY)) {
                            var dest = self.map.getDoorDestination(
                              gridX,
                              gridY,
                            );
                    ...
                    entity.onDeath(function () {
                      ...
                      entity.setSprite(
                        self.sprites[
                          entity instanceof Mobs.Rat ? "rat" : "death"
                        ],
                      );
    ...
    onCharacterUpdate: function (character) {
      ...
      if (character.isAttacking() && !character.previousTarget) {
        var isMoving = this.tryMovingToADifferentTile(character); // Don't let multiple mobs stack on the same tile
        // when attacking a player.

        if (character.canAttack(time)) {
          if (!isMoving) {
            // don't hit target if moving to a different
            // tile.
            if (
              character.hasTarget() &&
