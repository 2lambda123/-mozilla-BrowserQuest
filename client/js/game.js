...
define(
    ...
    function(InfoManager, BubbleManager, Renderer, Map, Animation, Sprite,
             AnimatedTile, Warrior, GameClient, AudioManager, Updater,
             Transition, Pathfinder, Item, Mob, Npc, Player, Character, Chest,
             Mobs, Exceptions, config) {
      var Game = Class.extend({
        ...
        setup : function($bubbleContainer, canvas, background, foreground,
                         input) {
        ...
        setStorage : function(storage) { this.storage = storage; },

        setRenderer : function(renderer) { this.renderer = renderer; },

        setUpdater : function(updater) { this.updater = updater; },

        setPathfinder : function(pathfinder) { this.pathfinder = pathfinder; },

        setChatInput : function(element) { this.chatinput = element; },

        setBubbleManager : function(
            bubbleManager) { this.bubbleManager = bubbleManager; },
        ...
        loadMap : function() {
          ...
          this.map.ready(function() {
            log.info("Map loaded.");
            var tilesetIndex =
                self.renderer.upscaledRendering ? 0 : self.renderer.scale - 1;
        ...
        initPlayer : function() {
          if (this.storage.hasAlreadyPlayed() && this.storage.data.player) {
            if (this.storage.data.player.armor &&
                this.storage.data.player.weapon) {
        ...
        initHurtSprites : function() {
          ...
          Types.forEachArmorKind(function(
              kind, kindName) { self.sprites[kindName].createHurtSprite(); });
        },

        initSilhouettes : function() {
          var self = this;

          Types.forEachMobOrNpcKind(function(
              kind, kindName) { self.sprites[kindName].createSilhouette(); });
        ...
        initAchievements : function() {
          ...
          this.achievements = {
            A_TRUE_WARRIOR :
                {id : 1, name : "A True Warrior", desc : "Find a new weapon"},
            INTO_THE_WILD : {
              id : 2,
              name : "Into the Wild",
            ...
            ANGRY_RATS : {
              ...
              desc : "Kill 10 rats",
              isCompleted :
                  function() { return self.storage.getRatCount() >= 10; }
            },
            SMALL_TALK : {
              id : 4,
              name : "Small Talk",
            ...
            FAT_LOOT :
                {id : 5, name : "Fat Loot", desc : "Get a new armor set"},
            UNDERGROUND : {
              id : 6,
              name : "Underground",
            ...
            TOMB_RAIDER :
                {id : 9, name : "Tomb Raider", desc : "Find the graveyard"},
            SKULL_COLLECTOR : {
              id : 10,
              name : "Skull Collector",
              desc : "Kill 10 skeletons",
              isCompleted :
                  function() { return self.storage.getSkeletonCount() >= 10; }
            },
            NINJA_LOOT : {
              id : 11,
              name : "Ninja Loot",
            ...
            HUNTER : {
              ...
              isCompleted :
                  function() { return self.storage.getTotalKills() >= 50; }
            },
            STILL_ALIVE : {
              id : 14,
              name : "Still Alive",
              ...
              isCompleted :
                  function() { return self.storage.getTotalRevives() >= 5; }
            },
            MEATSHIELD : {
              id : 15,
              name : "Meatshield",
        ...
        spritesLoaded : function() {
          if (_.any(this.sprites,
                    function(sprite) { return !sprite.isLoaded; })) {
            return false;
          }
          return true;
        },
        ...
        focusPlayer : function() { this.renderer.camera.lookAt(this.player); },
        ...
        addEntity : function(entity) {
          ...
          if (this.entities[entity.id] === undefined) {
            ...
            log.error("This entity already exists : " + entity.id + " (" +
                      entity.kind + ")");
          }
        },
        ...
        initAnimatedTiles : function() {
          ...
          this.forEachVisibleTile(function(id, index) {
            if (m.isAnimatedTile(id)) {
              var tile = new AnimatedTile(id, m.getTileAnimationLength(id),
        ...
        removeFromRenderingGrid : function(entity, x, y) {
          if (entity && this.renderingGrid[y][x] &&
        ...
        removeFromPathingGrid : function(x, y) { this.pathingGrid[y][x] = 0; },

        /**
         * Registers the entity at two adjacent positions on the grid at the
         * same time. This situation is temporary and should only occur when the
         * entity is moving. This is useful for the hit testing algorithm used
         * when hovering entities with the mouse cursor.
         *
        ...
        unregisterEntityPosition : function(entity) {
          if (entity) {
            ...
            if (entity.nextGridX >= 0 && entity.nextGridY >= 0) {
              this.removeFromEntityGrid(entity, entity.nextGridX,
                                        entity.nextGridY);
        ...
        registerEntityPosition : function(entity) {
          var x = entity.gridX, y = entity.gridY;
        ...
        loadAudio : function() { this.audioManager = new AudioManager(this); },
        ...
        entityIdExists : function(id) { return id in this.entities; },
        ...
        connect : function(started_callback) {
          var self = this,
              connecting =
                  false; // always in dispatcher mode in the build version
          ...
          if (config) {
            this.client.connect(
                config.dispatcher); // false if the client connects directly to
                                    // a game server
            connecting = true;
          }
          //>>excludeEnd("prodHost");
          ...
          if (!connecting) {
            this.client.connect(
                true); // always use the dispatcher in production
          }
          //>>includeEnd("prodHost");
          ...
          this.client.onEntityList(function(list) {
            ...
            self.obsoleteEntities = _.reject(self.entities, function(entity) {
              return _.include(knownIds, entity.id) ||
          ...
          this.client.onWelcome(function(id, name, x, y, hp) {
            ...
            self.player.dirtyRect =
                self.renderer.getEntityBoundingRect(self.player);

            setTimeout(
                function() { self.tryUnlockingAchievement("STILL_ALIVE"); },
                1500);
            ...
            self.player.onStartPathing(function(path) {
              var i = path.length - 1, x = path[i][0], y = path[i][1];
              ...
              if (self.renderer.mobile || self.renderer.tablet) {
                ...
                self.renderer.targetRect =
                    self.renderer.getTargetBoundingRect();
                self.checkOtherDirtyRects(self.renderer.targetRect, null,
            ...
            self.player.onCheckAggro(function() {
              self.forEachMob(function(mob) {
                if (mob.isAggressive && !mob.isAttacking() &&
            ...
            self.player.onAggro(function(mob) {
              if (!mob.isWaitingToAttack(self.player) &&
                  !self.player.isAttackedBy(mob)) {
                self.player.log_info("Aggroed by " + mob.id + " at (" +
                                     self.player.gridX + ", " +
                                     self.player.gridY + ")");
            ...
            self.player.onBeforeStep(function() {
              var blockingEntity = self.getEntityAt(self.player.nextGridX,
            ...
            self.player.onStep(function() {
              ...
              if ((self.player.gridX <= 85 && self.player.gridY <= 179 &&
                   self.player.gridY > 178) || (self.player.gridX <= 85 &&
                                                self.player.gridY <= 266 &&
                                                self.player.gridY > 265)) {
                self.tryUnlockingAchievement("INTO_THE_WILD");
              }

              if (self.player.gridX <= 85 && self.player.gridY <= 293 &&
                  self.player.gridY > 292) {
                self.tryUnlockingAchievement("AT_WORLDS_END");
              }

              if (self.player.gridX <= 85 && self.player.gridY <= 100 &&
                  self.player.gridY > 99) {
                self.tryUnlockingAchievement("NO_MANS_LAND");
              }

              if (self.player.gridX <= 85 && self.player.gridY <= 51 &&
                  self.player.gridY > 50) {
                self.tryUnlockingAchievement("HOT_SPOT");
              }

              if (self.player.gridX <= 27 && self.player.gridY <= 123 &&
            ...
            self.player.onStopPathing(function(x, y) {
              ...
              if (self.isItemAt(x, y)) {
                ...
                try {
                  ...
                  self.client.sendLoot(
                      item); // Notify the server that this item has been looted
                  ...
                  if (item.wasDropped &&
                      !_(item.playersInvolved).include(self.playerId)) {
              ...
              if (!self.player.hasTarget() && self.map.isDoor(x, y)) {
                ...
                if (_.size(self.player.attackers) > 0) {
                  setTimeout(
                      function() { self.tryUnlockingAchievement("COWARD"); },
                      500);
                }
                self.player.forEachAttacker(function(attacker) {
                  attacker.disengage();
                ...
                if (self.renderer.mobile || self.renderer.tablet) {
                  // When rendering with dirty rects, clear the whole screen
            ...
            self.player.onDeath(function() {
              ...
              self.player.animate("death", 120, 1, function() {
                ...
                self.removeFromRenderingGrid(self.player, self.player.gridX,
                                             self.player.gridY);

                self.player = null;
                self.client.disable();

                setTimeout(function() { self.playerdeath_callback(); }, 1000);
              });
            ...
            self.player.onHasMoved(function(
                player) { self.assignBubbleTo(player); });
            ...
            self.player.onSwitchItem(function() {
              self.renderer.getPlayerImage(function(playerImage) {
                self.storage.savePlayer(playerImage, self.player.getArmorName(),
                                        self.player.getWeaponName());
              });
            ...
            self.client.onSpawnItem(function(item, x, y) {
              log.info("Spawned " + Types.getKindAsString(item.kind) + " (" +
                       item.id + ") at " + x + ", " + y);
            ...
            self.client.onSpawnCharacter(function(entity, x, y, orientation,
                                                  targetId) {
                      ...
                      if (entity instanceof Character) {
                          ...
                          if (!entity.isDying) {
                            ...
                            if (entity instanceof Player) {
                              ...
                              if (self.map.isDoor(gridX, gridY)) {
                                var dest =
                                    self.map.getDoorDestination(gridX, gridY);
                        ...
                        entity.onRequestPath(function(x, y) {
                          var ignored = [ entity ], // Always ignore self
                              ignoreTarget = function(target) {
                                ...
                                // also ignore other attackers of the target
                                // entity
                                target.forEachAttacker(function(
                                    attacker) { ignored.push(attacker); });
                              };

                          if (entity.hasTarget()) {
                            ignoreTarget(entity.target);
                          } else if (entity.previousTarget) {
                            // If repositioning before attacking again, ignore
                            // previous target See: tryMovingToADifferentTile()
                        ...
                        entity.onDeath(function() {
                          ...
                          if (entity instanceof Mob) {
                            // Keep track of where mobs die in order to spawn
                            // their dropped items at the right position later.
                          ...
                          entity.isDying = true;
                          entity.setSprite(
                              self.sprites[entity instanceof Mobs.Rat
                                               ? "rat"
                                               : "death"]);
                          entity.animate("death", 120, 1, function() {
                            ...
                            self.removeFromRenderingGrid(entity, entity.gridX,
                                                         entity.gridY);
                          });

                          entity.forEachAttacker(function(
                              attacker) { attacker.disengage(); });
                          ...
                          // Upon death, this entity is removed from both grids,
                          // allowing the player to click very fast in order to
                          // loot the dropped item and not be blocked. The
                          // entity is completely removed only after the death
                          // animation has ended.
                          self.removeFromEntityGrid(entity, entity.gridX,
                                                    entity.gridY);
                          self.removeFromPathingGrid(entity.gridX,
                                                     entity.gridY);
                        ...
                        entity.onHasMoved(function(entity) {
                          self.assignBubbleTo(entity); // Make chat bubbles
                                                       // follow moving entities
                        });
            ...
            self.client.onDespawnEntity(function(entityId) {
              ...
              if (entity) {
                log.info("Despawning " + Types.getKindAsString(entity.kind) +
                         " (" + entity.id + ")");
            ...
            self.client.onEntityAttack(function(attackerId, targetId) {
              ...
              if (attacker && target && attacker.id !== self.playerId) {
                ...
                if (attacker && target instanceof Player &&
                    target.id !== self.playerId && target.target &&
                    target.target.id === attacker.id &&
                    attacker.getDistanceToEntity(target) < 3) {
                  setTimeout(
                      function() { self.createAttackLink(attacker, target); },
                      200); // delay to prevent other players attacking mobs
            ...
            self.client.onPlayerDamageMob(function(mobId, points) {
              ...
              if (mob && points) {
                self.infoManager.addDamageInfo(points, mob.x, mob.y - 15,
                                               "inflicted");
              }
            });
            ...
            self.client.onPlayerKillMob(function(kind) {
              ...
              if (kind === Types.Entities.SKELETON ||
            ...
            self.client.onPlayerChangeHealth(function(points, isRegen) {
              var player = self.player, diff, isHurt;
              ...
              if (player && !player.isDead && !player.invincible) {
                ...
                if (isHurt) {
                  player.hurt();
                  self.infoManager.addDamageInfo(diff, player.x, player.y - 15,
                                                 "received");
                  ...
                  self.infoManager.addDamageInfo("+" + diff, player.x,
                                                 player.y - 15, "healed");
                }
                self.updateBars();
              }
            ...
            self.client.onPlayerTeleport(function(id, x, y) {
              var entity = null, currentOrientation;
            ...
            self.client.onPopulationChange(function(worldPlayers,
                                                    totalPlayers) {
        ...
        /**
         * Sends a "hello" message to the server, as a way of initiating the
         * player connection handshake.
         * @see GameClient.sendHello
         */
        sendHello : function() { this.client.sendHello(this.player); },

        /**
         * Converts the current mouse position on the screen to world grid
         * coordinates.
         * @returns {Object} An object containing x and y properties.
         */
        getMouseGridPosition : function() {
          var mx = this.mouse.x, my = this.mouse.y, c = this.renderer.camera,
              s = this.renderer.scale, ts = this.renderer.tilesize,
              offsetX = mx % (ts * s), offsetY = my % (ts * s),
         ...
         */
        makePlayerGoTo : function(
            x, y) { this.makeCharacterGoTo(this.player, x, y); },
         ...
         * @param {Function} callback The function to call back (must accept one
         *     entity argument).
         */
         ...
         * : Lower 'y' value means higher depth. Note: This is used by the
         * Renderer to know in which order to render entities.
         */
          ...
          this.camera.forEachVisiblePosition(function(x, y) {
            if (!m.isOutOfBounds(x, y)) {
              if (self.renderingGrid[y][x]) {
                _.each(self.renderingGrid[y][x],
                       function(entity) { callback(entity); });
              }
            }
          ...
          if (m.isLoaded) {
            this.forEachVisibleTileIndex(function(tileIndex) {
              if (_.isArray(m.data[tileIndex])) {
                _.each(m.data[tileIndex],
                       function(id) { callback(id - 1, tileIndex); });
              } else {
        ...
        forEachAnimatedTile : function(callback) {
          if (this.animatedTiles) {
            _.each(this.animatedTiles, function(tile) { callback(tile); });
          }
        },

        /**
         * Returns the entity located at the given position on the world grid.
         * @returns {Entity} the entity located at (x, y) or null if there is
         *     none.
         */
        getEntityAt : function(x, y) {
          ...
          var entities = this.entityGrid[y][x], entity = null;
        ...
        getItemAt : function(x, y) {
          ...
          var items = this.itemGrid[y][x], item = null;

          if (_.size(items) > 0) {
            // If there are potions/burgers stacked with equipment items on the
            // same tile, always get expendable items first.
        ...
        /**
         * Returns true if an entity is located at the given position on the
         * world grid.
         * @returns {Boolean} Whether an entity is at (x, y).
         */
        isEntityAt : function(x,
                              y) { return !_.isNull(this.getEntityAt(x, y)); },

        isMobAt : function(x, y) { return !_.isNull(this.getMobAt(x, y)); },

        isItemAt : function(x, y) { return !_.isNull(this.getItemAt(x, y)); },

        isNpcAt : function(x, y) { return !_.isNull(this.getNpcAt(x, y)); },

        isChestAt : function(x, y) { return !_.isNull(this.getChestAt(x, y)); },
        ...
        findPath : function(character, x, y, ignoreList) {
          var self = this, grid = this.pathingGrid;
          ...
          if (this.pathfinder && character) {
            if (ignoreList) {
              _.each(
                  ignoreList,
                  function(entity) { self.pathfinder.ignoreEntity(entity); });
            }
            ...
            log.error("Error while finding the path to " + x + ", " + y +
                      " for " + character.id);
          }
          return path;
        },
        ...
        /**
         * Processes game logic when the user triggers a click/touch event
         * during the game.
         */
        click : function() {
          var pos = this.getMouseGridPosition(), entity;
          ...
          if (this.started && this.player && !this.isZoning() &&
              !this.isZoningTile(this.player.nextGridX,
                                 this.player.nextGridY) &&
              !this.player.isDead && !this.hoveringCollidingTile &&
        ...
        isMobOnSameTile : function(mob, x, y) {
          var X = x || mob.gridX, Y = y || mob.gridY,
              list = this.entityGrid[Y][X], result = false;
        ...
        getFreeAdjacentNonDiagonalPosition : function(entity) {
          ...
          entity.forEachAdjacentNonDiagonalPosition(function(x, y,
                                                             orientation) {
        ...
        tryMovingToADifferentTile : function(character) {
          ...
          if (attacker && target && target instanceof Player) {
            ...
            if (!target.isMoving() && attacker.isAdjacentNonDiagonal(target) &&
        ...
        onCharacterUpdate : function(character) {
          var time = this.currentTime, self = this;
          ...
          if (character.isAttacking() && !character.previousTarget) {
            var isMoving = this.tryMovingToADifferentTile(
                character); // Don't let multiple mobs stack on the same tile
                            // when attacking a player.

            if (character.canAttack(time)) {
              if (!isMoving) { // don't hit target if moving to a different
                               // tile.
                ...
                if (character.hasTarget() &&
                    character.target.id === this.playerId && this.player &&
        ...
        startZoningFrom : function(x, y) {
          ...
          if (this.renderer.mobile || this.renderer.tablet) {
            var z = this.zoningOrientation, c = this.camera,
                ts = this.renderer.tilesize, x = c.x, y = c.y,
                xoffset = (c.gridW - 2) * ts, yoffset = (c.gridH - 2) * ts;

            if (z === Types.Orientations.LEFT ||
                z === Types.Orientations.RIGHT) {
              x = (z === Types.Orientations.LEFT) ? c.x - xoffset
                                                  : c.x + xoffset;
            } else if (z === Types.Orientations.UP ||
            ...
            this.forEachVisibleEntityByDepth(function(
                entity) { entity.setDirty(); });
          } else {
        ...
        isZoning : function() { return !_.isNull(this.currentZoning); },
        ...
        say : function(message) { this.client.sendChat(message); },

        createBubble : function(id, message) {
          this.bubbleManager.create(id, message, this.currentTime);
        },

        destroyBubble : function(id) { this.bubbleManager.destroyBubble(id); },
        ...
        onGameStart : function(
            callback) { this.gamestart_callback = callback; },

        onDisconnect : function(
            callback) { this.disconnect_callback = callback; },

        onPlayerDeath : function(
            callback) { this.playerdeath_callback = callback; },

        onPlayerHealthChange : function(
            callback) { this.playerhp_callback = callback; },

        onPlayerHurt : function(
            callback) { this.playerhurt_callback = callback; },

        onPlayerEquipmentChange : function(
            callback) { this.equipment_callback = callback; },

        onNbPlayersChange : function(
            callback) { this.nbplayers_callback = callback; },

        onNotification : function(
            callback) { this.notification_callback = callback; },

        onPlayerInvincible : function(
            callback) { this.invincible_callback = callback },
        ...
        updateBars : function() {
          if (this.player && this.playerhp_callback) {
            this.playerhp_callback(this.player.hitPoints,
        ...
        onAchievementUnlock : function(
            callback) { this.unlock_callback = callback; },
        ...
        removeObsoleteEntities : function() {
          ...
          if (nb > 0) {
            ...
            log.debug(
                "Removed " + nb + " entities: " +
                _.pluck(_.reject(this.obsoleteEntities,
                                 function(id) { return id === self.player.id }),
                        'id'));
         ...
         *
         * For instance, to get rid of the sword cursor in case the mouse is
         * still hovering over a dying mob. Also useful when the mouse is
         * hovering a tile where an item is appearing.
         */
